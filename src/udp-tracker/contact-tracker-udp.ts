...
export const handleUDPTrackers = async (
  url: string,
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on("message", (msg) => {
      ...
      if (action === 0) {
        ...
        announceBuffer.writeBigUInt64BE(
          BigInt(headerAssemblyResults.downloaded),
          56
        );
        announceBuffer.writeBigUInt64BE(BigInt(headerAssemblyResults.left), 64);
        announceBuffer.writeBigUInt64BE(
          BigInt(headerAssemblyResults.uploaded),
          72
        );

...
const decodeDictionary = (
  ...
  start: number
): { decodedValue: Record<string, BencodeValue>; index: number } => {
  const object: Record<string, BencodeValue> = {}
...
const decodeString = (encodedString: Buffer, start: number) => {
  ...
  for (let i = start; i < encodedString.length; i++) {
    if (encodedString[i] === ':'.charCodeAt(0)) {
      ...
      decodedValue = encodedString.subarray(wordStartIndex, i + currentWordLength + 1)
...
const decodeNumber = (encodedString: Buffer, start: number) => {
  ...
  for (let i = start; i < encodedString.length; i++) {
    if (encodedString[i] === 'e'.charCodeAt(0)) {
      numEnd = i
      decodedValue = Number(encodedString.subarray(start + 1, numEnd).toString())
...
const encodeObject = (value: BencodeValue): string => {
  ...
  let finalStr = keys
    .map((key) => {
      return encodeString(key) + encode((value as { [key: string]: BencodeValue })[key])
    })
    .join('')
  return `d${finalStr}e`
}
